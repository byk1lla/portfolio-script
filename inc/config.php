...
class User extends SQL
{
  ...
  function getLastvisit($userid)
  {
    ...
    if (!$this->connect()) {
      ...
      try {
        $stmt = $this->conn->prepare("SELECT last_visit from uservisitors where userid = :user_id");
  ...
  function getuserPath($userid){
    ...
    if (!$this->connect()) {
      ...
      try {
        $stmt = $this->conn->prepare("SELECT pic_path as img from portfolio where user_id = :user_id");
        $stmt->bindParam(":user_id", $userid, PDO::PARAM_STR);
        $stmt->execute();
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['img'];
  ...
  function getvisit($userid)
  {
    ...
    try {
      $stmt = $this->conn->prepare("SELECT visitcount from uservisitors where userid = :user_id");
      $stmt->bindParam(":user_id", $userid, PDO::PARAM_STR);
      $stmt->execute();
      $result = $stmt->fetch(PDO::FETCH_ASSOC);
  ...
  function addVisit($userid)
  {
    $this->url = $_SERVER["REQUEST_URI"];
    $this->log = new Log();
    $this->log->logPath = "./inc/logs/";
    try {
      if (!$this->connect()) {
        $this->log->error($this->url, "Database is Offline!");
        throw new PDOException("Database Offline!");
      }
      $this->visit = $this->getvisit($userid);
      $stmt = $this->conn->prepare("SELECT COUNT(userid) AS count FROM uservisitors WHERE userid = :user_id");
      $stmt->bindParam(":user_id", $userid, PDO::PARAM_STR);
      $stmt->execute();
      $result = $stmt->fetch(PDO::FETCH_ASSOC);
      if ($result['count'] > 0) {
        $this->visit++;
        $sql = "UPDATE uservisitors SET visitcount = :visitcount WHERE userid = :userid";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':userid', $userid);
        $stmt->bindParam(':visitcount', $this->visit);
        $stmt->execute();
      } else {
        $sql = "INSERT INTO uservisitors (userid, visitcount, last_visit) VALUES (:userid, :visitcount, NOW())";
        $stmt = $this->conn->prepare($sql);
        $stmt->bindParam(':userid', $userid);
        $stmt->bindParam(':visitcount', $this->visit);
        $stmt->execute();
      ...
      $this->log->error($this->url, $this->errorDisplay($e));
      return $e;
    }
  }
  ...
  public function generateimgname($userid, $username, $ext)
  {
    $date = date("Ymd_His");
    $filename = $username . "-" . $userid . "_" . $date . ".$ext";
    return $filename;
  }
